...
async def scrape_product(asin: str,attribute: list, session: ClientSession, semaphore: asyncio.Semaphore) -> ProductInfo:
    ...
    async with semaphore:
        try:
            async with session.get(f"https://www.amazon.com/dp/{asin}", headers=headers) as response:
                ...
                parsed_data = parse_product(result, asin, attribute)
                if parsed_data['name'] == '':
...
async def run():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for asin, attributes in data.items():
            task = asyncio.create_task(scrape_product(asin=asin, attribute = attributes,session=session, semaphore=semaphore))
            tasks.append(task)
